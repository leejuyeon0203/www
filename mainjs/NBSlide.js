// JavaScript Document
 $(document).ready(function() {
   var position=0;  //최초위치 >> 목적지(left)
   //var movesize=150; //이미지 하나의 너비
   var movesize=$('.poster_box img').width(); //이미지 하나의 너비     
   var timeonoff;
   
    $('.poster_box ul').after($('.poster_box ul').clone());
       //슬라이드 겔러리를 한번 복제

   $('.button').click(function(event){ //버튼을 클릭하면 이벤트 타겟을 나가 변수의 $targe이다..
	var $target=$(event.target); //클릭한 해당버튼
	clearInterval(timeonoff);
	
	if($target.is('.m1')){ //왼쪽 버튼 클릭??
        
	     if(position==-3120){
	         $('.slide_gallery').css('left',0);
	          position=0;
	      }
		
	     position-=movesize;  // 150씩 감소
    	     $('.slide_gallery').stop().animate({left:position}, 'fast',
		  function(){							
		    if(position==-3120){
			   $('.slide_gallery').css('left',0);
			   position=0;
		    }
	      });
	}else if($target.is('.m2')){  //오른쪽 버튼 클릭??
	      if(position==0){
	           $('.slide_gallery').css('left',-3120);
	                 position=-3120;
                   }

          position+=movesize; // 150씩 증가
        
    	  $('.slide_gallery').stop().animate({left:position}, 'fast',
		  function(){	//앞에 animate다 계산한다음에 다시 if문을 계산한다 >>콜백함수						
		    if(position==0){
			   $('.slide_gallery').css('left',-3120);
			   position=-3120;
		    }
	       });
	  }
   });
   
   //최초 자동 슬라이드 기능
    timeonoff= setInterval(function () {
        position-=movesize;  // 150씩 감소
    	$('.poster_box').stop().animate({left:position}, 'fast',
	         function(){							
		    if(position==-3120){
			   $('.poster_box').css('left',0);
			   position=0;
		    }
	 });
     }, 2000);
   
 });

