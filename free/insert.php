...
<?
	...
	if ($mode=="modify")
	{
		...
		$sql = "update $table set subject='$subject', content='$content', kinds='$kinds' where num=$num";
		mysql_query($sql, $connect);  // $sql 에 저장된 명령 실행
	}
	else
	{
		...
		$sql .= " file_name_0, file_name_1, file_name_2, file_copied_0,  file_copied_1, file_copied_2, kinds) ";
		$sql .= "values('$userid', '$username', '$usernick', '$subject', '$content', '$regist_day', 0, '$is_html', ";
		$sql .= "'$upfile_name[0]', '$upfile_name[1]',  '$upfile_name[2]', '$copied_file_name[0]', '$copied_file_name[1]','$copied_file_name[2]','$kinds')";
		mysql_query($sql, $connect);  // $sql 에 저장된 명령 실행
	}
